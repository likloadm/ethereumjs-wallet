...
const isRunningInKarma = () => {
  return (
    ...
    (global as any).window.__karma__
  );
};
...
describe(".getPrivateKey()", function () {
  it("should work", function () {
    assert.strictEqual(
      fixtureWallet.getPrivateKey().toString("hex"),
      fixturePrivateKey
    );
  });
...
describe(".getPublicKey()", function () {
  it("should work", function () {
    assert.strictEqual(
      fixtureWallet.getPublicKey().toString("hex"),
      fixturePublicKey
    );
  });
});
...
describe(".verifyPublicKey()", function () {
  it("should return true if publicKey, privateKey pair is valid", function () {
    assert.strictEqual(
      fixtureWallet.verifyPublicKey(fixturePublicKeyBuffer),
      true
    );
  });
  it("should return false if publicKey, privateKey pair is invalid", function () {
    assert.strictEqual(
      fixtureWallet.verifyPublicKey(Buffer.alloc(64, 0)),
      false
    );
  });
});
...
describe(".toV3()", function () {
    ...
    await Promise.all(
      (
        ...
        assert.deepStrictEqual(
          JSON.parse(wStatic),
          JSON.parse(encFixtureWallet)
        );
        assert.deepStrictEqual(
          JSON.parse(wStatic),
          JSON.parse(encFixtureEthersWallet)
        );
        assert.deepStrictEqual(
  ...
  it("should work with empty salt", async function () {
    ...
    let wEthersStr = await new ethersWallet(
      fixtureWallet.getPrivateKeyString()
    ).encrypt(pw, {
      scrypt: { N: n, r: r, p: p },
    ...
    assert.deepStrictEqual(
      JSON.parse(wStr),
      JSON.parse(wEthersStr.toLowerCase())
    );
    assert.strictEqual(
      fixtureWallet.getPrivateKeyString(),
    ...
    wEthersStr = await new ethersWallet(
      fixtureWallet.getPrivateKeyString()
    ).encrypt(pw, {
      scrypt: { N: n, r: r, p: p },
      salt: salt,
  ...
  it("should fail for bad iv", function () {
    const pw = "test";
    const errStrLength =
      /^Error: Invalid iv, string must be 32 hex characters$/;
    ...
    assert.rejects(async function () {
      await fixtureWallet.toV3(pw, {
        iv: Buffer.from("ffffffffffffffffffffffffffffffffff", "hex"),
      });
    }, errBuffLength);
    assert.rejects(async function () {
      // @ts-ignore
  ...
  it("should fail for bad uuid", function () {
    const pw = "test";
    const errStrLength =
      /^Error: Invalid uuid, string must be 32 hex characters$/;
...
describe(".fromKryptoKit()", function () {
  ...
  it("should work with encrypted input (q-type)", async function () {
    ...
    assert.strictEqual(
      wallet.getAddressString(),
      "0x3c753e27834db67329d1ec1fab67970ec1e27112"
    );
  });
});
